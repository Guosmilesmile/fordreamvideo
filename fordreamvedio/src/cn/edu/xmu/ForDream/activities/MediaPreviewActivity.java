...
import cn.edu.xmu.ForDream.recorder.FrameImageView;
import cn.edu.xmu.ForDream.recorder.IsUtils;
import cn.edu.xmu.ForDream.recorder.ResourceUtils;
import cn.edu.xmu.ForDream.recorder.VideoView;
import cn.edu.xmu.ForDream.recorder.VideoView.OnPlayStateListener;
...
public class MediaPreviewActivity extends BaseActivity implements OnClickListener, OnPlayStateListener, OnPreparedListener {
	...
	private static String iscomnunty="0";
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		...
		iscomnunty  = getIntent().getStringExtra("iscommunty");
		if(getIntent().getStringExtra("videotype").equals("group"))
			isGroupVideo = true;
		else
			isGroupVideo = false;
		...
		findViewById(R.id.preview_title_left).setOnClickListener(this);
	...
	private String strVideoPath = Environment.getExternalStorageDirectory().getAbsolutePath() +"/VMS_" + System.currentTimeMillis()+ ".3gp";
		...
		if (!isFinishing() && mMediaObject != null) {
			new AsyncTask<Void, Void, Boolean>() {
				...
				protected void onPostExecute(Boolean result) {
					...
					if (result) {
						/** 序列化保存数据 */
						/** 转换成3gp格式，mp4转3gp不需要编码转换，只需要流数据重混re-muxing*/
						int tempresult = UtilityAdapter.FFmpegRun("", "ffmpeg -i " + mMediaObject.getOutputVideoPath() + " -c copy " + strVideoPath);
//						System.out.println("result:"+tempresult+",strVideoPath="+strVideoPath);
						
						long dateTaken = System.currentTimeMillis();
						String title = "VMS_" + dateTaken;
						String filename = title + ".jpg";
						String imagePath = Environment.getExternalStorageDirectory()+"/Android/data/" + getBaseContext().getPackageName()+"/video" + filename;
						FFMpegUtils.captureThumbnails(strVideoPath, imagePath, "200X200");
//						Toast.makeText(MediaPreviewActivity.this, getString(R.string.record_video_transcoding_success, mMediaObject.getOutputVideoPath()+","+imagePath), Toast.LENGTH_LONG).show();
						System.out.println("test:"+strVideoPath+","+imagePath);
						if(!isGroupVideo){
							
							Intent intent = null ;
							if("1".equals(iscomnunty)){
							  intent =  new Intent(MediaPreviewActivity.this, CompanySubmitActivity.class);
							}else{
							  intent = new Intent(MediaPreviewActivity.this, SubmitActivity.class);
							}
							Bundle b = new Bundle();
					        b.putString("path", strVideoPath);
							...
							MediaPreviewActivity.this.setResult(RESULT_OK); 
