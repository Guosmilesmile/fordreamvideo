...
public class CompanySubmitActivity extends Activity{
    ...
    private boolean sinax=false,tecentx=false,renrenx=false,doubanx=false;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.company_submit_activity);
        mLocationClient = ((LocationApplication)getApplication()).mLocationClient;
        positionTextView=(TextView)findViewById(R.id.company_submit_positionTextView);
        getPositionImageView=(ImageView)findViewById(R.id.company_submit_getPositionImage);
        videoImg=(ImageView)findViewById(R.id.company_submit_videoimg);
       ...
       //设置封面图片
        ...
        isLock=(LinearLayout)findViewById(R.id.company_submit_IsLock);
        isPublicImage=(ImageView)findViewById(R.id.company_submit_IsPublic);
        isPublicTextView=(TextView)findViewById(R.id.company_submit_isPublicTextView);
        
        editText=(EditText)findViewById(R.id.company_submit_editText1);
        
        addClassification=(LinearLayout)findViewById(R.id.company_submit_add_classification);
        //chooseGroup=(LinearLayout)findViewById(R.id.company_submit_choose_group);
        
        sinaweiboLayout=(LinearLayout)findViewById(R.id.company_submit_sinaweibo);
        tencentweiboLayout=(LinearLayout)findViewById(R.id.company_submit_tencentweibo);
        renrenLayout=(LinearLayout)findViewById(R.id.company_submit_renren);
        doubanLayout=(LinearLayout)findViewById(R.id.company_submit_douban);
        
        sinaweiboTextView=(TextView)findViewById(R.id.company_submit_sinaweiboTextView);
        tencentweiboTextView=(TextView)findViewById(R.id.company_submit_tencentweiboTextView);
        renrenTextView=(TextView)findViewById(R.id.company_submit_renrenTextView);
        doubanTextView=(TextView)findViewById(R.id.company_submit_doubanTextView);
        
        
        cancelTextView=(TextView)findViewById(R.id.company_submit_cancel_textView);
        sendTextView=(TextView)findViewById(R.id.company_submit_send_textView);
        uploadbar=(ProgressBar)findViewById(R.id.company_submit_uploadbar);
        resultView=(TextView)findViewById(R.id.company_submit_processresult);
        
        
       
        classificationTextView=(TextView)findViewById(R.id.company_submit_classificationTextView);
        //addShareGroupTextView=(TextView)findViewById(R.id.company_submit_addShareGroupTextView);
    ...
    private Handler handler = new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			case 1:
			{
				...
				if(uploadbar.getProgress() == uploadbar.getMax()){
					...
					Toast.makeText(CompanySubmitActivity.this, R.string.success, 1).show();
				}
			}
				break;
			//上传视频
			...
			case 4:
			{
				...
				if(whichpop==CLASSFICATION_POP)
				{
					...
					if(addShareGroupTextView.getText().toString().equals(getString(R.string.submit_shareGroupOriginalString)))
					{
					...
					{
						addShareGroupTextView.setText(addShareGroupTextView.getText().toString()+";"+chooseString);
	 ...
	 private final class ClickListener implements View.OnClickListener, PlatformActionListener, Callback
	{
		...
		public void onClick(View v) {
			...
			case R.id.company_submit_getPositionImage:
			{
				...
				break;
			case R.id.company_submit_IsLock:
			{
				//Log.i("IsLock", "IsLock");
				...
				else if(isPublicTextView.getText().toString().equals(getString(R.string.private_friend))) {
				...
				break;
			case R.id.company_submit_add_classification:
			{
				...
				break;
			//case R.id.company_submit_choose_group:
			//{
				//showPopwindow(ChooseGroup_POP);
			//}
				
				//break;				
			case R.id.company_submit_sinaweibo:
			{
				Platform sina = ShareSDK.getPlatform (CompanySubmitActivity.this,SinaWeibo.NAME);
				sina. setPlatformActionListener (this); // 设置分享事件回调
				if (sina.isValid()&&sinaweiboTextView.getCurrentTextColor()==getResources().getColor(R.color.choose_color)){
					sina.removeAccount();
				}
				else if(sina.isValid()&&sinaweiboTextView.getCurrentTextColor()==getResources().getColor(R.color.unchoose_color))
				{
				...
				if(sinaweiboTextView.getCurrentTextColor()==getResources().getColor(R.color.unchoose_color))
				{
				...
				break;	
			case R.id.company_submit_tencentweibo:
			{
				Platform tencentwb = ShareSDK.getPlatform (CompanySubmitActivity.this,TencentWeibo.NAME);
				tencentwb. setPlatformActionListener (this); // 设置分享事件回调
				if (tencentwb.isValid()&&tencentweiboTextView.getCurrentTextColor()==getResources().getColor(R.color.choose_color)){
					tencentwb.removeAccount();
				}
				else if(tencentwb.isValid()&&tencentweiboTextView.getCurrentTextColor()==getResources().getColor(R.color.unchoose_color))
				{
					showToastMessage("已授权成功~~~~");
					tecentx=true;
				}
				...
				if(tencentweiboTextView.getCurrentTextColor()==getResources().getColor(R.color.unchoose_color))
				{
				...
				break;					
			case R.id.company_submit_renren:
			{
				Platform renren = ShareSDK.getPlatform (CompanySubmitActivity.this,Renren.NAME);
				renren. setPlatformActionListener (this); // 设置分享事件回调
				if (renren.isValid()&&renrenTextView.getCurrentTextColor()==getResources().getColor(R.color.choose_color)){
					renren.removeAccount();
				}
				else if(renren.isValid()&&renrenTextView.getCurrentTextColor()==getResources().getColor(R.color.unchoose_color))
				{
					showToastMessage("已授权成功~~~~");
					renrenx=true;
				}
				...
				if(renrenTextView.getCurrentTextColor()==getResources().getColor(R.color.unchoose_color))
				{
			...
			case R.id.company_submit_douban:
			{
				Platform douban = ShareSDK.getPlatform (CompanySubmitActivity.this,Douban.NAME);
				douban. setPlatformActionListener (this); // 设置分享事件回调
				if (douban.isValid()&&doubanTextView.getCurrentTextColor()==getResources().getColor(R.color.choose_color)){
					douban.removeAccount();
				}
				else if(douban.isValid()&&doubanTextView.getCurrentTextColor()==getResources().getColor(R.color.unchoose_color))
				{
					showToastMessage("已授权成功~~~~");
					doubanx=true;
				}
				...
				if(doubanTextView.getCurrentTextColor()==getResources().getColor(R.color.unchoose_color))
				{
				...
				break;	
				//点击发送
			case R.id.company_submit_send_textView:
			{
				...
				else */if(positionTextView.getText().toString().equals(getString(R.string.getposition)))
				{
				...
				break;	
			case R.id.company_submit_cancel_textView:
			{
				finish();
			}
		...
		public boolean handleMessage(Message msg) {
			...
			switch (msg.arg1) {
				case 1: {
					...
					if(plat.getName().equals("Douban"))
						doubanx=true;
					Toast.makeText(CompanySubmitActivity.this, text, Toast.LENGTH_SHORT).show();
				}
				break;
				case 2: {
					// 失败
	 		...
	 		new Thread(new Runnable() {	
				public void run() {
			 		...
			 		try {
			 			...
			 			map.put("videoIsPublic", isPublicTextView.getText().toString());
			 			map.put("videoClassfication", channelMap.get(classificationTextView.getText().toString()));
			 			map.put("videoShareGroup",shareGroupId);
			 			map.put("videoPosition", positionTextView.getText().toString());
			 			map.put("iscommunty","1");
						String result=HttpUtils.postRequest(FinalUrl.LogVideoUrl, map);
	 	...
	 	private void initPopwindow(int which)
	 	{
			...
			//初始化分类弹窗
	 		case CLASSFICATION_POP:
	 		{
	 			...
	 			ArrayAdapter<String> adapter=new ArrayAdapter<String>(CompanySubmitActivity.this, R.layout.choose_classfication_item, R.id.choose_classfication_item_textView,classficationlist);
	 			choose_classfication_popupWindow=new ChoosePop(R.layout.choose_classfication_popwindow, CompanySubmitActivity.this, handler,adapter,CLASSFICATION_POP);
	 		...
	 		case ChooseGroup_POP:
	 		{
	 			...
	 			ArrayAdapter<String> groupadapter=new ArrayAdapter<String>(CompanySubmitActivity.this, R.layout.choose_classfication_item, R.id.choose_classfication_item_textView,grouplist);
	 			choose_group_popWindow=new ChoosePop(R.layout.choose_classfication_popwindow, CompanySubmitActivity.this, handler,groupadapter,ChooseGroup_POP);
	 		}
			default:
				break;
			}
	 	...
	 	private void showPopwindow(int which)
	 	{
			...
			case CLASSFICATION_POP:
			{
	    		...
	    		choose_classfication_popupWindow.showAtLocation(addClassification,Gravity.NO_GRAVITY, location[0], location[1]+classificationTextView.getHeight());
			}				
				break;
			case ChooseGroup_POP:
			{
	    		...
	    		choose_group_popWindow.showAtLocation(chooseGroup,Gravity.NO_GRAVITY, location[0], location[1]+addShareGroupTextView.getHeight());
			}
...
//			}
		...
		private void showShare(boolean silent, String platform, boolean captureView) {
		   ...
		   // oks.setImageUrl(FinalUrl.VIDEOIMAGE_URL+imagenameString);
			...
			oks.show(CompanySubmitActivity.this);
   ...
   private void initVedioClassification()
   {
	   channelMap=new HashMap<String, String>();
		new Thread(new Runnable() {	
			public void run() {
				String result=HttpUtils.getRequest(FinalUrl.GetClassficationUrl);
				...
				try {
					...
					for(int i = 0 ; i < array.length() ; i++){
						...
						channelMap.put(String.valueOf(channeljsonObject.get("channelname")), 
									   String.valueOf(channeljsonObject.get("id")));
					}
	   ...
	   groupMap=new HashMap<String, String>();
		new Thread(new Runnable() {	
			public void run() {
				...
				String result=null;
				try {
					result = HttpUtils.postRequest(FinalUrl.GetGroupListUrl,postmapHashMap);
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
				...
				try {
					...
					for(int i = 0 ; i < array.length() ; i++){
						...
						groupMap.put(String.valueOf(groupjsonObject.get("name")), 
   ...
   //显示提示
 	public void showToastMessage(String msg) {
 		Toast.makeText(CompanySubmitActivity.this, msg, Toast.LENGTH_LONG).show();
 	}	
